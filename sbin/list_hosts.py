...
if neither --authen nor --author are specified we list from authentication and database """)
    ...
    if options.authen:
        sys.stdout.write("\nHosts in the authen backend. : \n")
        print('\n'.join(['\t%s' % host for host in authen.list_hosts()]) + '\n')
    if options.author:
        if len(args) == 0:
            ...
            if not domainID:
                sys.exit(5)
        sys.stdout.write("\nHosts in the db backend : \n")
        print('\n'.join(['\t%s\t%s; %s; %s; %s' % vals for vals in db.list_hosts(domain=domainID)]) + '\n')
